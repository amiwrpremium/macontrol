...
func parseCall(req *http.Request) Call {
	...
	if ct == "multipart/form-data" {
		// Cap the whole request body before ParseMultipartForm so a
		// malformed upload can't OOM the test process. The 8 MiB
		// memory cap below is what gosec G120 wants flagged; the
		// outer MaxBytesReader keeps the on-the-wire body bounded
		// too. This is a test harness — the only callers are our
		// own test code uploading screenshots / artwork — but
		// defensive depth is cheap.
		req.Body = http.MaxBytesReader(nil, req.Body, 16<<20)
		if err := req.ParseMultipartForm(8 << 20); err == nil { // #nosec G120 -- body capped via MaxBytesReader above; test-harness only.
