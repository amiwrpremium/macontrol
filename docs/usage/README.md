...
that appears attached to the bot's reply after you send `/menu`.

This group covers every interactive surface:

## What's here

- **[UX model](ux-model.md)** — how inline keyboards edit messages
  in place, multi-step flows for free-text input, and confirm
  sub-keyboards for destructive actions.
...
The bot has two distinct interaction shapes:

| Shape | Example | When you'll see it |
|---|---|---|
| **Inline keyboard** | The 10-category home grid attached to `/menu`'s reply, or `−5 / −1 / MUTE / +1 / +5` under a 🔊 Sound dashboard | After every command and on every dashboard |
| **Flow** | Bot asks "Enter target volume (0-100)" and waits | When you tap an action that needs free-text input |

Inline keyboards stay attached to their message and edit the message
text when you press a button — no new message is sent for each tap.
Flows take over the chat for up to 5 minutes or until you reply with
`/cancel`.
